  ...
  return (
    <>
      ...
      {hovered && (
        ...
        >
          {hovered.properties?.name ?? "Unknown country"}
        </div>
      )}
      ...
      <div style={styles.controls}>
        <span>◉</span> {countryCount || "—"} countries
      </div>
    </>
  );
}
...
const styles: Record<string, React.CSSProperties> = {
  ...
  eyebrow: { fontSize: 11, letterSpacing: "0.2em", fontWeight: 700, opacity: 0.55 },
  title: { margin: "6px 0 4px", fontSize: "clamp(30px, 4vw, 54px)", lineHeight: 1, letterSpacing: "-0.04em" },
  subtitle: { margin: 0, fontSize: 13, opacity: 0.65 },
  searchHint: {
    marginTop: 4, padding: "9px 13px",
  ...
  loading: {
    ...
    borderRadius: 999, background: "rgba(2,8,16,.78)",
  ...
  error: {
    ...
    maxWidth: 420, padding: "14px 16px", borderRadius: 14,
    background: "rgba(100,20,20,.72)",
    border: "1px solid rgba(255,100,100,.2)", fontSize: 12,
  },
  tooltip: {
    ...
    backdropFilter: "blur(12px)", fontSize: 12,
    fontWeight: 650, whiteSpace: "nowrap",
  },
  panel: {
    position: "absolute", top: 32, right: 32,
    width: "min(330px, calc(100vw - 64px))", padding: 22,
    borderRadius: 20, background: "rgba(4,12,22,.82)",
  ...
  close: {
    ...
    border: 0, borderRadius: "50%", background: "rgba(255,255,255,.08)",
    color: "#fff", cursor: "pointer", fontSize: 20,
  },
  flag: {
    display: "block", width: 58, height: 38, objectFit: "cover",
    borderRadius: 7, marginBottom: 18,
  },
  panelEyebrow: { fontSize: 10, letterSpacing: "0.18em", opacity: 0.45 },
  countryName: { margin: "5px 0 20px", fontSize: 28, letterSpacing: "-0.03em" },
  details: {
    display: "grid", gap: 1, overflow: "hidden",
    borderRadius: 12, background: "rgba(255,255,255,.055)",
  },
  detail: {
  ...
  panelNote: { margin: "16px 0 0", fontSize: 11, lineHeight: 1.5, opacity: 0.5 },
  controls: {
    position: "absolute", left: 32, bottom: 28,
    padding: "8px 12px", borderRadius: 999,
    background: "rgba(255,255,255,.055)",
    border: "1px solid rgba(255,255,255,.1)",
    backdropFilter: "blur(10px)", fontSize: 11, opacity: 0.65,
  },
};
